...
{
    // carrega relacionamentos principais do livro
    $book->load(['author', 'publisher', 'category']);

    // carrega TODOS os empréstimos desse livro (com usuário), ordenados do mais novo
    $borrowings = \App\Models\Borrowing::where('book_id', $book->id)
        ->with('user')
        ->orderByDesc('borrowed_at')
        ->get();

    // lista de usuários para o select de empréstimo (mantém como antes)
    $users = User::all();

    return view('books.show', compact('book', 'users', 'borrowings'));
}
